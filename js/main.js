...
const downloadButton = document.getElementById('downloadButton');
  ...
  const fileUrl = 'images/1.jpg'; // Replace this with the URL of your file
  ...
  if (xhr.status === 200) {
  const blob = new Blob([xhr.response], { type: 'application/jpg/png/pdf' }); // Change the type accordingly
  const url = window.URL.createObjectURL(blob);
  
  const a = document.createElement('a');
  a.href = url;
  a.download = 'images/1.jpg'; // Change the filename accordingly
